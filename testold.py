...
def getPPV(myResults,repoBefore,repurposed_names):
    ...
    repoRes['length'] = list(repoRes['indications'].apply(getWeight))
    myResults['length'] = list(myResults['indications'].apply(getWeight))
    repoRes2 =  repoRes[repoRes['Success']==True]

    trueWeight = repoRes2['length'].sum()
    allWeight = myResults['length'].sum()
    #print(str(len(repoRes2)/(len(myResults)+0.0)))
    print(len(repoRes2)/(allWeight+0.0))
...
ratio=99 #this will give all the data in fullTableDF
reporesratio,fullTableDF = getResultsLess(ratio)
fullTableDF = fullTableDF[fullTableDF['ref']<float('inf')]
fullTableDF_under10 = fullTableDF[fullTableDF['ref']<10]
meanRatio = fullTableDF_under10['ref'].mean()
stdRatio = fullTableDF_under10['ref'].std()
for ratio in [(meanRatio-2*stdRatio),(meanRatio-stdRatio),meanRatio,(meanRatio+stdRatio),(meanRatio+2*stdRatio)]:
    ...
    reporesratio,fullTableDF = getResultsLess(ratio)
    getPPVAll(reporesratio,repoBefore,repurposed_names)
print("**********************")
for ratio in [(meanRatio - 2 * stdRatio), (meanRatio - stdRatio), meanRatio, (meanRatio + stdRatio),(meanRatio + 2 * stdRatio)]:
    print("ratio: "+str(ratio) +' More:')
    reporesratio,fullTableDF = getResultsMore(ratio)
    getPPVAll(reporesratio,repoBefore,repurposed_names)

ratio = fullTableDF_under10['ref'].min()
reporesratio,fullTableDF = getResultsMore(ratio)
tmp = getPPV(reporesratio,repoBefore,repurposed_names)

#check only distances in MOA (MolVec)
fullStmp = pn.DataFrame()
for testcase in testcases:
    fullStmp = fullStmp.append(getSDFMol2Vec(drugs_S, distances_S, indices_S, testcase))
fullStmp_under05 = fullStmp[fullStmp['SrelDist']<=0.5]
meanRatio = fullStmp_under05['SrelDist'].mean()
stdRatio = fullStmp_under05['SrelDist'].std()

for ratio in [(meanRatio-2*stdRatio),(meanRatio-stdRatio),meanRatio,(meanRatio+stdRatio),(meanRatio+2*stdRatio)]:
    print("ratio: "+str(ratio) +' Less:')
    reporesratio,fullTableDF = getResultsSOnlyLess(ratio)
    getPPV(reporesratio,repoBefore,repurposed_names)
print("**********************")
for ratio in [(meanRatio - 2 * stdRatio), (meanRatio - stdRatio), meanRatio, (meanRatio + stdRatio),(meanRatio + 2 * stdRatio)]:
    print("ratio: "+str(ratio) +' More:')
    reporesratio,fullTableDF = getResultsSOnlyMore(ratio)
    getPPV(reporesratio,repoBefore,repurposed_names)

#check only distances in MOA (SemRep)
fullStmp = pn.DataFrame()
for testcase in testcases:
    fullStmp = fullStmp.append(getSDF(drugs_S, distances_S, indices_S, testcase))
meanRatio = fullStmp['SrelDist'].mean()
stdRatio = fullStmp['SrelDist'].std()

for ratio in [(meanRatio-2*stdRatio),(meanRatio-stdRatio),meanRatio,(meanRatio+stdRatio),(meanRatio+2*stdRatio)]:
    print("ratio: "+str(ratio) +' Less:')
    reporesratio,fullTableDF = getResultsSOnlySemRepLess(ratio)
    getPPV(reporesratio,repoBefore,repurposed_names)
print("**********************")
for ratio in [(meanRatio - 2 * stdRatio), (meanRatio - stdRatio), meanRatio, (meanRatio + stdRatio),(meanRatio + 2 * stdRatio)]:
    print("ratio: "+str(ratio) +' More:')
    reporesratio,fullTableDF = getResultsSOnlySemRepMore(ratio)
    getPPV(reporesratio,repoBefore,repurposed_names)
...
fullStmp = pn.DataFrame()
for testcase in testcases:
    fullStmp = fullStmp.append(getTDF(drugs_S, distances_S, indices_S, testcase))
meanRatio = fullStmp['TrelDist'].mean()
stdRatio = fullStmp['TrelDist'].std()

for ratio in [(meanRatio-2*stdRatio),(meanRatio-stdRatio),meanRatio,(meanRatio+stdRatio),(meanRatio+2*stdRatio)]:
    print("ratio: "+str(ratio) +' Less:')
    reporesratio,fullTableDF = getResultsTOnlyLess(ratio)
    getPPV(reporesratio,repoBefore,repurposed_names)
print("**********************")
for ratio in [(meanRatio - 2 * stdRatio), (meanRatio - stdRatio), meanRatio, (meanRatio + stdRatio),(meanRatio + 2 * stdRatio)]:
    print("ratio: "+str(ratio) +' More:')
    reporesratio,fullTableDF = getResultsTOnlyMore(ratio)
    getPPV(reporesratio,repoBefore,repurposed_names)
